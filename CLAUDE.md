...
- `src/pages/index.tsx` is the only page. Its `sections` array is the single source of truth for section order **and** the corner navigation: each entry is `{ Component, id?, label? }`, where `id` must equal the uppercase `id` on that section's heading (e.g. `id="WORK"`), which nav clicks scroll to, and `label` is the localized nav text. Adding, removing, or reordering a section means editing this array (and giving the section a matching heading `id`). The heading label shown on the page may differ from the component name (`Skill` → `SKILLS`, `Blog` → `FEATURED`).
- Scroll focus: `index.tsx` wraps each section in a `Box` and tracks the active one (the last section whose top has passed `min(innerHeight / 3, 300px)`, or the last section at page bottom). Inactive sections are dimmed via opacity. The same active label is passed to `CornerNavs` to highlight the nav, so `CornerNavs` has no scroll logic of its own.
- `CornerNavs` renders two layouts switched by CSS at the `md` breakpoint: desktop corner elements, and below `md` a fixed top bar (`MOBILE_HEADER_HEIGHT`) with a full-screen top `Drawer` menu for nav and socials. All in-page jumps go through `src/utils/scrollToSection.ts`, which offsets for whichever header is showing; menu clicks scroll only after the drawer's exit transition, because the open drawer locks page scroll. Section vertical padding is applied from `index.tsx` via `'& > div > div'`, overriding each section's own padding.
...
- i18n: Next.js built-in i18n routing (`next.config.mjs`) with `en` (default, at `/`) and `zh-TW` (at `/zh-TW`); `localeDetection` is off so the browser language never redirects. `src/utils/i18n.ts` exports `useLocale()` → `{ locale, t }`, and every component keeps its copy inline as `Localized<T>` values (`{ en: ..., 'zh-TW': ... }`) passed through `t()`. Product, company, and tech names stay in English in the Chinese copy. The language toggle sits next to the scheme toggle in `CornerNavs`. Section heading `id`s stay English in both locales. The PDFs are English-only.
